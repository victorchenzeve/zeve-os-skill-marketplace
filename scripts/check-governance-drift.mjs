...
const expectedCounts = { skills: 25, plugins: 5, agents: 5 };
...
for (const entry of skills.filter(item => item.category === 'core')) {
  if (entry.maturity !== 'stable' || entry.evaluation.status !== 'repository-passed') errors.push(`${entry.id}: core governance evidence drift`);
}
for (const entry of skills.filter(item => item.category !== 'core')) {
  if (entry.maturity !== 'experimental') errors.push(`${entry.id}: domain skill requires real-world evidence before stable`);
}
...
if (!read('README.md').includes('25 个 Skill、5 个领域插件、5 个代理')) errors.push('README asset counts are stale');
